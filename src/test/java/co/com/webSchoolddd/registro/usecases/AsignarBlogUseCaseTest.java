...
import co.com.webSchoolddd.useCases.AsignarBlogUseCase;
...
public class AsignarBlogUseCaseTest {
    @Mock
    private DomainEventRepository repository;

    @Test
    @DisplayName("Testing asignarCurso")
    void asignarBlog() {
        //arrange
        EscuelaId escuelaId = EscuelaId.of("xxx");

        BlogId blogId = BlogId.of("xxz");
        Author author = new Author("Miguel", "migule@mail.com");
        Contenido contenido = new Contenido("La importancia de React", new Date());

        //act
        var command = new AsignarBlog(
                escuelaId,
                blogId,
                contenido,
                author
        );
        var usecase = new AsignarBlogUseCase();
        ...
        //assert
        var event =(BlogAsignado)events.get(0);
        Assertions.assertEquals("webSchoolddd.registro.escuela.examenAsignado", event.type);
        Assertions.assertEquals("xxz", event.getBlogId().value());
